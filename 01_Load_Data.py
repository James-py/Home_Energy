...
def fix_heating(df_mod, df, circuit, cutoff, offset, multiplier1, spike):
    df_mod.loc[:, circuit] = df.loc[:, circuit]
    # # when signal is > cutoff
    rows = df[circuit] > cutoff
    df_mod.loc[rows, circuit] = df.loc[rows, circuit] + offset + (df.loc[rows, circuit] - cutoff) * multiplier1
...
if False:
    lines_plot(
        kW_mod.loc[:, ["Heat_Beds", "Test_MTU"]],  # .resample('1d').sum(),
    ...
    # plt.close(fig='all')
    lines_plot(
        kW.loc[:, ["Heat_Beds", "Test_MTU"]],  # .resample('1d').sum(),
        weather_df,  # .resample('1d').mean(),
...
if False:
    # Bed Rooms
    lines_plot(
        kW.loc[:"2023-10-28", ["Heat_Beds", "Test_MTU"]],
        weather_df,
    ...
    lines_plot(
        kW_mod.loc[:"2023-10-28", ["Heat_Beds", "Test_MTU"]],
        weather_df,
    ...
    # %%Living Room
    lines_plot(
        kW.loc[:"2023-11-13", ["Heat_LvRm", "Test_MTU"]],
        weather_df,
    ...
    lines_plot(
        kW_mod.loc[:"2023-11-13", ["Heat_LvRm", "Test_MTU"]],
        weather_df,
...
def fix_double_pole(df_mod, df, circuit, c1, c2, multiplier):
    df_mod.loc[df[c1] > 0, circuit] = df.loc[df[c1] > 0, [c1, c2]].sum(axis=1) * multiplier
...
if False:
    lines_plot(kW_tot_compare, "MTU and Spyder Total Power Comparison", drop_list=[], legend=True, ylab="Power (kW)")
...
lines_plot(
    kW.loc["2023-10-10 10:00":"2023-10-28 13:20", ["Heat_Beds", "Test_MTU"]],
...
lines_plot(
    kW_mod.loc["2023-10-10 10:00":"2023-10-28 13:20", ["Heat_Beds", "Test_MTU"]],
    weather_df.loc["2023-10-10 10:00":"2023-10-28 13:20", :],
    "Heat_Beds kW_mod Oct 10 to 28",
    start_date="2023-10-10",
    ylab="kW",
...
lines_plot(
    kW.loc["2023-10-28 13:30":"2024-03-09 8:32", ["Heat_LvRm", "Test_MTU"]],
    weather_df,
    "Heat Living Room kW Oct 28",
    ylab="kW",
    start_date="2023-10-28",
    legend=True,
)
...
lines_plot(
    kW_mod.loc["2023-10-28 13:30":"2024-03-09 8:32", ["Heat_LvRm", "Test_MTU"]],
    weather_df,
    "Heat Living Room kW_mod Oct 28",
    ylab="kW",
    start_date="2023-10-28",
...
lines_plot(
    kW_mod.loc[:MTU_change_date, ["DHWHP_Spy", "Test_MTU"]],
    weather_df.loc[:MTU_change_date, :],
    "DHW spy vs test MTU - Power",
    ylab="kW",
...
lines_plot(
    kWh.loc[:MTU_change_date, ["DHWHP_Spy", "Test_MTU"]],  # .resample('1d').sum(),
    weather_df.loc[:MTU_change_date, ["Temp (°C)", "Rel Hum (%)"]],  # .resample('1d').mean(),
    "DHW spy vs test MTU - hourly energy",
    ylab="kWh",
    start_date="2024-03-12",
    legend=True,
    vs_temp=True,
...
lines_plot(
    kWh.loc[:, ["Heat_Beds", "Test_MTU"]],
    weather_df,
    "Bedroom Heat Mod vs Test MTU - hourly energy",
    ylab="kWh",
    start_date=MTU_change_date,
    legend=True,
    vs_temp=True,
)
...
# %% One-off lines Plot
lines_plot(
    kW_mod.loc[:, ["K_Fridge"]],  # .resample('1d').sum(),
    weather_df,  # .resample('1d').mean(),
...
fridge_day = kWh.loc["2025-09-01":today, ["K_Fridge"]].resample("1d").sum()
